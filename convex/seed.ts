...
export const seedPromotions = seedMutation({
  ...
  handler: async (ctx) => {
    ...
    await ctx.db.insert('promotions', { title: '2+1 ֆիլտրերի վրա', description: 'Գնիր 2 ֆիլտր և ստացիր 1-ը նվեր։ MANN-FILTER, Bosch, Knecht։', images: ['/promotions/filters.svg'], imageUrl: '/promotions/filters.svg', discountPercent: 33, startDate: now, endDate: now + 30 * 24 * 60 * 60 * 1000, isActive: true, createdAt: now });
    await ctx.db.insert('promotions', { title: 'Կոճղակների զեղչ', description: 'Մինչև -30% զեղչ արգելակման կոճղակների վրա։ Brembo, TRW, ATE։', images: ['/promotions/brakes.svg'], imageUrl: '/promotions/brakes.svg', discountPercent: 30, startDate: now, endDate: now + 45 * 24 * 60 * 60 * 1000, isActive: true, createdAt: now });
    await ctx.db.insert('promotions', { title: 'Բատարիաների գարնանային զեղչ', description: '-15% Varta, Bosch և Banner բատարիաների վրա։ Սահմանափակ ժամանակով։', images: ['/promotions/batteries.svg'], imageUrl: '/promotions/batteries.svg', discountPercent: 15, startDate: now, endDate: now + 40 * 24 * 60 * 60 * 1000, isActive: true, createdAt: now });
    return 'Seeded 5 promotions';
  },
});
